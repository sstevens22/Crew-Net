...
const queryEmpsInfo =
  "SELECT employees.id, employees.first_name, employees.last_name, roles.title, roles.salary, departments.dept FROM employees LEFT JOIN roles ON employees.role_id = roles.id LEFT JOIN departments ON roles.dept_id = departments.id;";
...
const queryEmpChoices =
  'SELECT CONCAT(employees.first_name, " ", employees.last_name) AS empName FROM employees;';
...
const addEmp = () => {
  connection.query(queryRoles, (err, res) => {
    ...
    let roles = res;
    connection.query(
      `SELECT employees.first_name, employees.last_name, employees.role_id, roles.title FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.is_mangr=TRUE;`,
      (err, res) => {
        ...
        prompt([
          ...
          let empRoleManager = roles.find(
            (role) => role.id === answers.newRoleManager
          );
          connection.query(
    ...
    prompt([
      ...
      connection.query(queryDepts, (err, res) => {
        ...
        const departments = res.find(
          (departments) => departments.dept === answers.newRoleDept
        );
        connection.query(
          queryAddRole,
          {
    ...
    connection.query("SELECT roles.title FROM roles", (err, res) => {
      ...
      prompt([
        ...
        connection.query(queryRoles, (err, res) => {
          if (err) throw err;
          let roles = res;
          let updatedRole = roles.find(
            (role) => role.title === answers.updateRole
          );
  ...
  connection.query(queryEmpChoices, (err, res) => {
    ...
    prompt([
      ...
      connection.query(queryEmpsBasic, (err, res) => {
        ...
        const employees = res.find(
          (employees) =>
            employees.first_name === fullName[0] &&
            employees.last_name === fullName[1]
        );
        connection.query(
          queryDeleteEmp,
          {
